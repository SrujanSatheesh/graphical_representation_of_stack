...
lass button {   
int x1,y1,x2,y2;   
int state; char str[10];  
public: button() {  
}   
button(int x11,int y11,int x22,int y22,char *str1) {  
 x1=x11; y1=y11;   
x2=x22; y2=y22;   
state=1; strcpy(str,str1); //copying it to stack  
}   
void draw();   
void togglestate();   
...
void drawstring(float x,float y,char *string) //to type strings  
 {   
...
int i;  
if (st.stempty()) drawstring(10,10,"Stack Is Empty!");   
else {   
for (i = st.top; i >= 0; i--)   
st.s[i].draw();   
}   
...
drawstring(100,500,"GRAPHICAL REPRESENTATION OF STACK");   
glColor3f(1,0.5,0);  
btn1.draw();   
btn2.draw();  
displaystacknqueue();   
...
static int itemno; y=600-y;  
 if(btn==GLUT_LEFT_BUTTON && state == GLUT_DOWN) {   
if(btn1.insidebutton(x,y)) {  
 btn1.togglestate();   
if(!st.stfull()) {   
printf("Enter the number") ;   
scanf("%d",&itemno); st.push(itemno);   
}   
}   
if(btn2.insidebutton(x,y)) {  
 btn2.togglestate();   
if(!st.stempty()) st.pop();  
}  
}   
...
if(btn2.insidebutton(x,y)) { btn2.togglestate(); }  
}  
 glutPostRedisplay();  
 } void init() {   
